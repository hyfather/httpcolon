    ...
    if (data != null && data.instances != null) {
      ...
      const _links = instances.map((item) => {
        return (
          <Anchor
            className={
              item.timestamp === active ? classes.linkActive : classes.link
            }
            key={item.timestamp}
            variant="text"
            onClick={(event) => {
    ...
    <Navbar height="100%" width={{ sm: 300 }} p="md" className={classes.navbar}>
      <Navbar.Section grow>
        <Group spacing="xs" position="right">
          ...
          >
            <span user-select="none">HISTORY</span>
          </Text>
          <ActionIcon
            mr={10}
            variant="filled"
      ...
      <Navbar.Section className={classes.footer}>
        <Center>
          <Stack>
            {themeSwich}
            <Anchor
              href="https://github.com/hyfather/httpcolon/tree/master/json"
              target="_blank"
