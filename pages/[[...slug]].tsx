  ...
  return {
    header: {
      ...
      backgroundImage:
        theme.colorScheme === 'dark'
          ? theme.fn.gradient({
              from: theme.colors.dark[7],
              to: theme.colors.dark[6],
              deg: 0,
            })
          : theme.fn.gradient({
              from: theme.colors.gray[0],
              to: theme.colors.grape[0],
              deg: 20,
            }),
    },
    ...
    card: {
      ...
      backgroundImage:
        theme.colorScheme === 'dark'
          ...
          : theme.fn.gradient({
              from: theme.colors.grape[1],
              to: theme.colors.blue[1],
              deg: 200,
            }),
      fontSize: theme.fontSizes.sm,
      // color: theme.white,
...
export default function HomePage(props) {
  ...
  const makeAPICall = (
    ...
    fetch(slugURL)
      ...
      .then((data) => {
        ...
        setSlugLoader(1);
        setCopyURL(
          window.location.href.toString().replace('?refresh=true', '')
        );
  ...
  function reSlug(refresh: boolean = false) {
    ...
    if (slug1) {
      ...
      makeAPICall(
        encodedSlug,
        router.query.method ? router.query.method.toString() : 'GET',
        refresh
      );
      refreshNavBar();
    }
        ...
        return (
          <a
            className={cx(classes.link, {
              [classes.linkActive]: item.timestamp === active,
            })}
            key={item.timestamp}
            onClick={(event) => {
          ...
          >
            <span>
              {' '}
              <IconClock size={16} stroke={1} /> {timestamp.toLocaleString()}
            </span>
          </a>
        );
      });
        ...
        <Header height={50} p="xs" className={classes.header}>
          <Group spacing="sm" position="apart" mt={1}>
            <Group spacing="sm" align="vertical" mt={-2}>
              ...
              >
                <span user-select="none">HTTP:COLON</span>
              </Text>
              <Badge
                color="gray"
                variant="outline"
                ...
                <tbody>
                  <tr className={classes.tableRow}>
                    ...
                    <td className={classes.tableValue}>
                      <ColonizeFormV2
                        inputValue={eValue}
                        setInputValue={setEValue}
                        method={response.method}
                        onSubmit={reSlugTo}
                        isEditing={colonizeFormEditable}
                        setIsEditing={setColonizeFormEditable}
                      />
                    </td>
                  </tr>
                  <tr className={classes.tableRow}>
                    ...
                    <td className={classes.tableValue}>
                      <Group spacing="xs">
                        <Select
                          ...
                          rightSection={
                            colonizeFormEditable ? (
                              <IconEdit size={12} stroke={2} />
                            ) : (
                              <ActionIcon
                                onClick={() => setColonizeFormEditable(true)}
                                color="grape"
                                variant="outline"
                                size="xs"
                              >
                  ...
                  <tr className={classes.tableRow}>
                    ...
                    <td className={classes.tableValue}>
                      <Badge
                        color={
                          response.latency < 200
                            ? 'green'
                            : response.latency < 600
                              ? 'yellow'
                              : 'red'
                        }
                        radius="sm"
            ...
            <div id="headers">
              {!!response.payload && (
                <TableSort
                  ...
                  setHeaderMetadata={setHeaderData}
                  updateTable={updateTable}
      ...
      {!slug && (
        <Group
          ...
          sx={{
            background:
              theme.colorScheme === 'dark'
                ? theme.fn.gradient({
                    from: theme.colors.dark[8],
                    to: theme.colors.dark[8],
                    deg: 1,
                  })
                : theme.fn.gradient({
                    from: theme.colors.gray[0],
        ...
        >
          ...
          >
            ...
            <Anchor
              href="https://github.com/hyfather/httpcolon"
              target="_blank"
            >
